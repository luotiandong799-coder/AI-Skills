...
## 强制动作（每次会话开始必做）
1. **读取完整规则全文**：`C:\Users\26719\.workbuddy\AGENTS.md`（完整版，本文件仅为摘要）。不读不得开始实质性工作。
2. **读取插件动态报告**：`D:\腾讯AI\plugin-watch-report.md`。有候选 → 报候选；无候选 → 一句话带过。
3. **每个任务开工前先扫技能清单**（用户 2026-09-12 强制，原话"一定要默认让所有可用的 skill 工具看任务运行啊，你光安装有什么用"）：过一遍系统注入的可用技能清单 → 判定强/弱/无关 → **强相关立即调用 Skill 工具加载**（禁止凭记忆复述冒充加载）→ 一次加载 1–3 个。**装了不调用 = 没装。** 收尾自检"有没有装了没用的"。详见 AGENTS.md ★ 节。

## 三件套默认执行（用户级 · 长期 · 每会话）
以下三项逻辑在 WorkBuddy 下**由我直接执行、每个会话默认生效、用户级（跨项目跨会话）**，**不依赖任何插件或 hook 运行**：
- **ponytail**（决策·少写代码）：写代码/实现功能前走 YAGNI 决策阶梯，最后才写最简代码；非代码任务不套用。
- **wb-max-token-saver**（输出·压缩废话）：答案优先、无套话、输出只留要点、结论基于证据、完整任务不缩水。**含防跑偏硬规则（2026-09-13 强制默认）：多步编号、禁开场白、禁跑题、一次一件事。**
- **wb-context-compressor**（上下文·聚焦相关）：只注入相关信息、长输出摘要不丢关键错误/数据、保护用户指定内容、验证/测试/安全步骤不省。**含工具输出沙箱（2026-09-13 强制默认）：长输出先落盘只读结论。**
- **规约先行（2026-09-13 强制默认）**：多步/实现任务开工前三件事——先问清需求（禁脑补）→ 再拆计划（一步一验证）→ 测试先行。载体 `wb-spec-driven`（与 obra/superpowers 等价）。
- **强制默认技能扩展（2026-09-13，盲测 18/18 后用户指定，同日精简）**：以下技能场景命中即**必须加载**、不依赖逐次判别——`wb-debug-loop`（异常即排障循环，禁先改再猜）/ `wb-execute-discipline`（批量目标+周期任务零省略、失败换路攻坚，loop-engineering 删除后为执行纪律唯一承载）/ `wb-spec-driven`（多步实现先立规约）/ `wb-artifact-verification`（交付物先独立验证）/ `agent-guild`（身份·记忆·跨 agent 交接）。`browser-automation` 经用户指定**保持触发式**，不入默认。详见 AGENTS.md 3.6。
- **已删技能（2026-09-13 用户指令）**：`web-search`（P9，联网搜索由内置 WebSearch/WebFetch 承担）、`wb-loop-engineering`（P17，周期任务自进化框架随之取消；其执行纪律内容并入 wb-execute-discipline）。删除后引用已全部清理。
- **默认技能 GitHub 同步（2026-09-13 用户强制"以后默认执行的技能要放在我的github仓库 从这个开始"）**：默认执行技能每次新增/修改/删除后，同步 `D:\腾讯AI\skills` → `D:\AI技能仓库`（remote `github.com:luotiandong799-coder/AI-Skills`，SSH over 443 直连无需 VPN，git.exe 用 `C:\Users\26719\.workbuddy\binaries\PortableGit\versions\1.2.0\cmd\git.exe`）并 commit+push；AGENTS.md 一并同步。
- **载体**：AGENTS.md 第 1–3、3.5、3.6 条（完整规则）+ 用户级原生技能 `wb-ponytail` / `wb-max-token-saver` / `wb-context-compressor` / `wb-spec-driven`（位于 `~/.workbuddy/skills/`）。
- **本段即独立生效**，即使不先读 AGENTS.md 也照此执行。Claude Code 侧才是真插件（靠 hooks 跑）；(2026-09-10：`caveman` 已与 `wb-max-token-saver` 合并，目录已删除，不再单独存在。)

## 核心规则摘要
- **技能必须被调用才算存在（2026-09-12）**：每个任务开工先扫全量技能清单，命中的**立即加载**再干活；凭记忆复述 ≠ 加载；一次 1–3 个；装了不调用 = 没装。新装技能必须当轮接线（写进相邻技能的分工表 / description 触发词）。
...
- **改动必须显式说明（2026-09-14 用户强制，原话"以后改了什么要说出来"）**：凡改了**用户面文件**（`AGENTS.md`、`MEMORY.md` 用户级/项目级、技能 `SKILL.md`/脚本/触发词、自动化 prompt、配置与环境如注册表/junction/VPN），当轮回复**必须点明"改了什么、为什么"**，禁止默默改完；改全局规则文件还要给出"改前→改后"与回滚方式。**本规则自我适用**（把本规则写进规则文件这件事也要说出来 + 附 grep/md5 证据）。不适用：只读操作、临时文件、纯产物交付。详见 AGENTS.md 第 0.8 条。
- **用户指令必须执行且优先于我的判断（2026-09-15 用户强制，原话"必须听我的指令"）**：用户指令 = 必须执行的动作项，我的判断只能用于"怎么做得更好"，**不能用于"要不要做"**。① 不擅自判"没必要/已做过/重复"而跳过用户点名的目标；② 不擅自替换用户指定的做法（可同时提更好方案，但不得默默替换）；③ 冲突时除安全/合规/不可逆风险外一律按用户指令执行并说明分歧；④ **被墙站点需要 VPN 时自己开**：`D:\爬楼梯用的\Athena\一元机场.VIP.exe`，代理 `127.0.0.1:33210`（33233 不可用），**用完必须关进程 + ProxyEnable 还原 0 + flushdns + 二次复核**；⑤ 改动必须落到指定仓库 `D:\AI技能仓库` 并 commit + push。详见 AGENTS.md 第 0.9 条。
...
- **agent 内容唯一落点 = `D:\腾讯AI\`（2026-09-11 用户强制，原话"以后关于你的内容全部放到D盘关于你的文件夹中 不要乱放 以前的也先移过去"＋"C 盘用户 267 的文件你的内容全部搬到 D 盘腾讯AI文件夹中…然后把原来C盘重复的删掉"）**：已归拢 —— `~/.workbuddy`（junction→`D:\腾讯AI\.workbuddy`，原 C 路径仍可用）、`~/.claude`/`~/.codebuddy`/`~/.codex`/`~/.sheetagent`/`~/.copilot`/`~/.local`/`~/.workbuddy-key-fallback`/`~/.cache/codex-runtimes`/`AppData\Roaming\WorkBuddy`/`AppData\Local\CodeBuddyExtension`（均 junction→D）、`plugin-watch-report.md`、旧产出 `D:\yt`+`D:\腾讯AI\YT、`→`D:\腾讯AI\yt\`。写脚本/临时产物同样落 `D:\腾讯AI\`。
  ...
  - 自检工具：`D:\腾讯AI\_check-links.ps1`（读 `_links-map.json` 校验/自修复全部 17 个链接；实测出现过 junction 中途被外部删除）。**改链接只改 JSON，不改脚本。** 报告见 `D:\腾讯AI\yt\docs\2026-09-11_agent内容搬迁报告.md`。
  ...
  - **junction-first 模式（2026-09-12 用户强制，原话"物理落盘也全部在 D 盘，C 盘只剩一个零字节指针 这就是避免C盘再现内容的根本了"）**：**任何新的 agent 数据目录，第一步就在 D 盘建实体 + 登记 `_links-map.json` + 跑 `_check-links.ps1` 建链**，不让真实数据在 C 盘落过地。已应用：`~/.agent-guild` → `WorkBuddy数据\agent-guild`（17/17 链接正常，`ag doctor` 全过）。若数据已误落 C 盘：bash `cp -r` 复制到 D（PS 的 C→D Copy/Move 会被沙箱静默终止，bash 可用）→ `diff -r` 验证一致 → bash `mv` 腾空 C 路径 → 登记 map → 跑脚本补链。**C→D junction 创建只有用户侧脚本能做**（沙箱拦我的 New-Item Junction）。
...
## 用户专业背景（2026-09-08 自述 · 2026-09-12 更正）
- **所学专业：供应链管理；学历层次：本科**（2026-09-12 用户明确"我是本科"，此前误记为硕士，已改）。
- **学校与资源入口**：西安欧亚学院 · 工商管理学院 · 供应链管理本科。`https://lms.eurasia.edu/user/index#/` → 应用中心 → 学术资源 → **中国知网**（学校免登录入口，跳转 `https://www.cnki.net`）。应用中心还有：论文管理、论文助手、全球案例、specialSCi、畅想之星。
- **已掌握的课程方法**（从 LMS 课程作业评语确认）：FMEA（含 RPN）、AHP-FCE；做过西安银桥乳业（生鲜乳制品/冷链）供应链风险识别与管控案例。论文选题建议延续这条线。
- 学术写作默认按**中文本科论文**口径处理：引用格式 **GB/T 7714**，方法难度以本科可独立完成为准（问卷/案例/二手数据 > 博弈建模/PLS-SEM/系统动力学）；需要 2–5 篇外文文献。
