---
name: web-scrape
description: 抓取网页并输出干净的 Markdown、纯文本、HTML 或结构化 JSON。当用户要读取、抓取、整理某个网站或 AI Agent 站点的内容，需要批量采集公开网页数据、提取列表页字段，或要求"把某个网页/网站抓下来"时使用。不适用于需要登录、付费墙后的内容。也不适用平台类内容（小红书/推特/B站/LinkedIn/YouTube/雪球/播客）与全网调研——那些走 `agent-reach`。
---
...
**相邻分工（避免与 `agent-reach` 抢同一条触发词）**
| 需求 | 走哪 |
|---|---|
| 普通网页 / 文档站 / 列表页 → Markdown，或批量列表页出结构化字段 | **本技能**（本地引擎，三档难度 + `--fields`，无需登录） |
| 平台内容（小红书 / 推特 / B站 / V2EX / Reddit / LinkedIn / YouTube / 雪球） | `agent-reach`（平台路由 + 登录态后端） |
| 全网调研（多平台并行收集再汇总） | `agent-reach` |
| 需要打开页面、点选、填表、复用登录态 | `browser-automation` |

一句话：**本技能是「抓取引擎」，`agent-reach` 是「平台路由器」。**
