...
# ---------- 破坏性操作守卫（本地补丁，2026-09-12） ----------

def assert_safe_skill_target(target: Path) -> Path:
    """在「先 rmtree 再 copytree」之前调用：拒绝明显不该被删除的目标。

    防的是 --target 传错（如 ~、盘根、仓库根）导致的不可逆损失。
    判定标准：目标必须看起来是一个 skill pack（顶层含 SKILL.md），
    且不得是文件系统根或用户主目录。
    """
    p = Path(target).expanduser()
    try:
        rp = p.resolve()
    except OSError as exc:  # 断链、权限等
        raise SystemExit(f"[guard] 无法解析目标路径 {p}: {exc}") from exc

    if rp == rp.parent:
        raise SystemExit(f"[guard] 拒绝操作文件系统根目录: {rp}")
    home = Path.home().resolve()
    if rp in (home, home.parent):
        raise SystemExit(f"[guard] 拒绝操作用户主目录或其上级: {rp}")
    if not (rp / "SKILL.md").is_file():
        raise SystemExit(
            f"[guard] {rp} 顶层没有 SKILL.md，不像 skill pack，拒绝删除/覆盖。"
            f" 若确认无误，请手工处理该目录"
        )
    return rp
