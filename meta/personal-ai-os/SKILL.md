...
## 九·五、执行预算 Execution Budget（用户 2026-09-20 强制 · 硬上限）

§九 说"什么时候停"，本节说"最多花多少"。**预算是硬上限，不是建议值**——到达任一项即转入收尾。

### 1. 分级预算（按任务复杂度定档，先定档再动手）
...
**不允许一律上最高档**。默认先按简单档试，不够再升档——升档要说明理由，不能因为"怕不全面"直接开复杂档。

### 2. 各项硬上限

| 项 | 上限 | 到顶动作 |
|---|---|---|
| 单个 MCP / 外部工具失败重试 | **2 次**（第 1 次快速修正参数重试，第 2 次换路径） | 连续失败 → 切换备用工具/方案，或停手报告 |
| 同一来源 / 文件 / 数据重复读取 | **1 次** | 复用已有结果，不重读 |
| 无新增信息的连续调用 | **2 次** | 判定"已到信息边界"，停止扩大范围 |
| 搜索轮次 | **2 轮** | 第 2 轮无新增关键信息 → 停止 |
| 并行任务每批并行度 | **5 个** | 超过则分批串行 |
| 同一目标原样重试 | **禁止**（详见 §十 与 `wb-execute-discipline`「失败 ≥2 次必根因诊断」） | 先辨错误类型：瞬时→退避重试；确定性（403/404/认证/bot 拦截）→ 直接换路 |

### 3. 到顶不是失败，是收尾信号

**达到任一上限时，必须：优先输出当前已有结果 + 明确说明未完成的部分。**
**禁止**为了"更完整"而突破上限；也禁止拿"上限到了"当借口半途交付而不说明缺口。两者都违反纪律。

### 3.5 两条显式例外通道（2026-09-20 首次实战后补，来源：D4 执行实测）

定档发生在**任务开始时**，但执行中可能撞上计划外情况。不许靠"违规完成"，要给**显式通道**：

| 情形 | 处置 |
|---|---|
| **执行中发现计划外缺陷，且不修会造成持续损害**（如：修 A 时发现同类静默故障 B，不回写机制下次照旧） | **允许突破上限继续修**，但必须同时：① 在交付物里**显式声明超支**（原档位、实际用量、超支原因）；② 把成因写成预算条目的补充（本条即由此产生） |
| **用户在同一任务中途追加了需求** | **重新定档**（按追加后的整体范围算），不是沿用旧档位硬扛 |

**不许拿这两条当免死金牌**：例外必须**可点名**——说清"发现的是什么计划外缺陷""为什么不能延后"。说不出具体缺陷名 = 不是例外，是失控。

### 4. 最短路径原则（省的是调用次数，不是质量）
...
### 5. 上下文纪律（与 `wb-context-compressor` 同源）
