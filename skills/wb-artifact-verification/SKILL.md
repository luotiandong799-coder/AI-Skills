...
## 复核者的能力、收敛与"一次通过"：别把弱标准读成好结果（来源：GitHub `2dmurali/review-loop-skill` 的 `review-loop`，2026-09-16 实拉）
一个 worker/reviewer 迭代环把"复核为什么没用"讲成了可判定的几条，与既有的独立性规则互补（既有规则管**独立不独立**，这条管**够不够格**）：
- **复核者的判定能力必须 ≥ 产出该产物的复杂度**：原文硬规则 "The Reviewer must always be EQUAL TO or MORE POWERFUL than the Worker"。弱于产出方的复核者挑不出复杂逻辑里的回归，**只会给高分**。→ 判据：**复核者能不能独立推导出这个产物里的关键决策？** 推导不出就升级（换更强的复核者 / 换人 / 加机械检查），**不许拿弱复核者的通过当质量证据**。
- **评分停滞要归因到执行方能力，不是"再跑一轮"**：原文判据——**同一条反馈连续 2 轮分数不涨** = 执行方太弱，不是迭代次数不够。处置三选一：**升级执行方 / 临时降低门槛并向用户声明缺口 / 把任务拆小后分别跑**。**加轮次不在选项里**。→ 与 `wb-execute-discipline` §停滞放行 的分工：那条管"什么时候允许停"，这条管"停之前该做哪个归因"。
- **复核者没有前轮记忆，历史结论必须显式注入**：子 agent 复核者看不到上一轮说过什么；**不把"已修复 / 已驳回及理由"一起交给它，循环会在两三个问题之间来回震荡**。→ 每轮派发都带上这份清单，而不是只丢产物。
- **"一次就满分"先怀疑标准松，再怀疑成果好**：设**最小复核轮次下限**（默认 2），**未满下限就满分 → 判检查标准太松或产物太小**，不判"质量优秀"。判据：**这条标准能不能被一个已知不合格的样本挂掉？**（与 §断言判别力 / §控制项证明的是评分器 同族。）
- **评分维度必须能写出"什么样算不合格"**：模糊标准（"写得好一点""再稳一些"）直接导致分数通胀；**写不出不合格样子的维度不进评分表**。
