...
from cangjie_common import WriterLock, snapshot_dir, assert_safe_skill_target  # noqa: E402
...
def apply_patch(target: Path, patch_dir: Path, snapshots: Path) -> int:
    ...
    assert_safe_skill_target(target)  # 校验失败会 rmtree+还原，先确认目标确实是 skill pack
...
def restore(target: Path, snapshot: Path) -> int:
    ...
    with WriterLock(target):
        if target.exists():
            assert_safe_skill_target(target)
            shutil.rmtree(target)
