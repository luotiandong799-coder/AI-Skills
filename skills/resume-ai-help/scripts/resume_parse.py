#!/usr/bin/env python3
"""
简历解析确定性脚本 (resume_parse.py) —— 1:1 还原 goffer `analysisResume` 的解析/清洗/归一化。
...
goffer 解析逻辑（`PersonalResumeFacadeImpl#analysisResume` / `简历模块配置.json`）变更时，
必须同步更新本文件与 dyson `src/main/resources/mcp/resume/resume-module-config.json` /
